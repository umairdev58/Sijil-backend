...
const prepareProductFilters = (rawProduct, rawProducts) => {
  ...
  return { productFilters, hasPotato, treatAsExactMatch };
};
...
const getCustomerOutstanding = async (req, res) => {
  try {
    ...
    if (groupBy === 'product') {
      customerOutstanding.forEach((productGroup) => {
        ...
        customers.forEach((customer) => {
          if (customer.status === 'overdue') {
            overdueCustomers += 1;
          } else if (customer.status === 'partially_paid') {
      ...
      overdueCustomers = customerOutstanding.filter((customer) => customer.status === 'overdue').length;
      partiallyPaidCustomers = customerOutstanding.filter((customer) => customer.status === 'partially_paid').length;
      unpaidCustomers = customerOutstanding.filter((customer) => customer.status === 'unpaid').length;
    }

    const summary = {
      totalOutstanding: customerOutstanding.reduce((sum, customer) => sum + customer.totalOutstanding, 0),
      totalCustomers: totalCustomersSummary,
      overdueCustomers,
