  ...
  try {
    ...
    if (expense.isAutoGenerated) {
      return res.status(400).json({
        ...
        message: 'Auto-generated expense cannot be edited'
      });
    }

    expense.description = description.trim();
    expense.amount = ceilToTwoDecimals(parsedAmount);
...
const removeExpense = async (req, res) => {
  try {
    ...
    const expenseIndex = statement.expenses.findIndex((expense) => expense._id?.toString() === expenseId);
    if (expenseIndex === -1) {
      return res.status(404).json({
        success: false,
        error: 'Expense not found',
    ...
    const expense = statement.expenses[expenseIndex];
    if (expense.isAutoGenerated) {
      return res.status(400).json({
        success: false,
        error: 'Protected expense',
    ...
    statement.expenses.splice(expenseIndex, 1);
    statement.markModified('expenses');
    statement.updatedBy = req.user.id;
    await statement.save();
...
// @access  Private
const generatePDFFromPayload = async (req, res) => {
  try {
    const { containerNo, products, expenses, companyName, srNo } = req.body;
    ...
    // Create statement object from payload
    const statement = {
    ...
    console.error('Generate PDF from payload error:', error);
